...
proc handleMixMessages(
  ...
  # PHASE-A INSTRUMENTATION: every sphinx packet arriving at this mix node.
  info "[PHASE-A] mix-recv: incoming sphinx",
    fromPeerId = fromPeerId,
    selfPeerId = mixProto.mixNodeInfo.peerId,
    receivedLen = receivedBytes.len
  ...
  of Exit:
    ...
    # PHASE-A INSTRUMENTATION: this node is the exit hop, about to dispatch
    # the unwrapped payload to the final destination via exit layer.
    info "[PHASE-A] mix-exit: dispatching to destination",
      selfPeerId = mixProto.mixNodeInfo.peerId,
      codec = deserialized.codec,
      destination = $processedSP.destination,
      surbCount = surbs.len,
      msgLen = message.len
...
proc forgetSurbsForIncoming*(
    mixProto: MixProtocol, incoming: AsyncQueue[seq[byte]]
) =
  ## Drops every connCreds entry whose `incoming` queue ref matches the
  ## given queue. Called from MixEntryConnection.closeImpl when a SURB
  ## round-trip never returns (timeout / cancellation) so SURB credentials
  ## don't accumulate indefinitely. The successful-reply path at
  ## handleMixMessages already deletes via igroup; this is the timeout twin.
  var toDelete: seq[SURBIdentifier] = @[]
  for id, cred in mixProto.connCreds:
    if cred.incoming == incoming:
      toDelete.add(id)
  for id in toDelete:
    mixProto.connCreds.del(id)
...
proc sendPacket(
  ...
  info "[INSTR] sendPacket entry", peerId = peerId, multiAddress = $multiAddress, label = label
  ...
  let packetToSend = mixProto.generateAndAppendProof(sphinxPacket.serialize(), label).valueOr:
    info "[INSTR] sendPacket generateAndAppendProof FAILED", err = error
    return err(error)

  info "[INSTR] sendPacket proof appended, about to writeLp", packetLen = packetToSend.len
  ...
  try:
    ...
    info "[INSTR] sendPacket writeLp DialFailedError", peerId = peerId, addr = $multiAddress, msg = exc.msg
    ...
    info "[INSTR] sendPacket writeLp LPStreamError", peerId = peerId, addr = $multiAddress, msg = exc.msg
    mix_messages_error.inc(labelValues = [label, "SEND_FAILED"])
    return err(fmt"Failed to write to next hop ({peerId}, {multiAddress}): {exc.msg}")
  except CancelledError as exc:
    raise exc

  info "[INSTR] sendPacket writeLp completed OK", peerId = peerId, addr = $multiAddress
