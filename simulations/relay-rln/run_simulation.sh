...
REOF
    ...
    # Retry node startup up to 3 times if subscribe call deadlocks (occasional FFI race)
    MAX_ATTEMPTS=3
    for attempt in $(seq 1 $MAX_ATTEMPTS); do
        ...
        # Wait for all expected calls to succeed
        ...
        if [ "${N:-0}" -ge "$EXPECTED_CALLS" ]; then
            ...
            log "  Node $i ready ($N/$EXPECTED_CALLS calls, attempt $attempt) PID: $NODE_PID"
            break
        fi

        if [ "$attempt" -lt "$MAX_ATTEMPTS" ]; then
            log "  Node $i attempt $attempt: $N/$EXPECTED_CALLS calls — killing PID $NODE_PID and retrying"
            ...
            INSTANCE_PIDS+=($NODE_PID)
            echo "  WARNING: Node $i: $N/$EXPECTED_CALLS calls after $MAX_ATTEMPTS attempts (PID: $NODE_PID)"
        fi
    done
...
echo ""
echo "[5/6] Waiting for messages (node 0 sends after 120s delay + 10 msgs at 5s intervals)..."
echo ""

# Total wait: 120s peer discovery + 50s sending + 20s buffer = 190s
# Poll every 10s with progress
for tick in $(seq 1 19); do
    sleep 10
    TS=$(date '+%H:%M:%S')
